...
面向日常读论文、做对比、写总结的场景：上传 PDF 入库并建立索引，用 AI 生成结构化要点，支持文献库级问答与多篇横向对比，并可导出 Markdown。

**主要能力**

- PDF 上传与文本抽取（需带可选中文字层的 PDF）
- 后端调用大模型（API Key 仅在后端使用，不进入浏览器）
- 单篇：PRD 结构化总结（研究问题/方法/结论/关键词）
- 多轮问答：基于**整个文献库**检索回答（非单文献）
- 多篇：主题、差异、机会与建议
- 阅读目标（是否深读 / 讲解 / 落地）与读者角色会影响输出结构
- 回答反馈闭环：`👍/👎` + “问题类型错误”标签，落地 `feedback.jsonl`
- 导出 Markdown 报告
...
- `eval.js` 每日评估脚本（对比今天/昨天效果）
- `.env.example` 环境变量模板
- `.github/workflows/nightly-eval.yml` GitHub 每晚自动评估
...
- `MAX_FULLTEXT_CHARS` 单篇全文输入长度上限（默认 600000）。
- `RAG_RELAXED_MIN_SIMILARITY` 宽松召回阈值（概述类问题兜底）。
- `DOC_PROFILE_MAX_CHARS` 通篇理解画像的输入截断长度。
...
若把程序部署在自己的 **VPS**，请将 `.env` 中密钥只留在服务器上，勿提交到 Git；置于 Nginx 等反向代理之后时务必设置 **`TRUST_PROXY=1`**（否则限流会按代理 IP 统计）。
...
4. 使用终端输出的 `https://……trycloudflare.com` 直接访问即可。
...
1. 上传 PDF 并点击“解析 PDF 并建立索引”
2. 查看并确认“文献信息”中的标题与摘要（已自动回填）
3. 生成 PRD 结构化总结
4. 在底部对话区直接对文献库多轮提问
5. 对回答做 `👍/👎` 与“问题类型错误”反馈
6. 多篇对比：加入列表后点“生成对比”
...
- `POST /api/documents`：上传 PDF 入库并建索引（返回 `document_id`）
- `GET /api/library/status`：文献库状态（文献数、可检索数、画像覆盖等）
- `POST /api/library/reset`：清空文献库内存与持久化缓存（需传 `{"confirm":"yes"}`）
- `POST /api/documents/:id/summarize`：单文献 PRD 结构化总结
- `POST /api/library/query`：文献库级问答（跨文献检索）
- `POST /api/compare`：多文献对比（`papers` 至少两篇）
- `POST /api/feedback`：写入反馈 JSONL（`vote` + `wrong_question_type`）

## 数据持久化说明

- 文献库与会话记忆会自动持久化到 `data/library-store.json`。
- 反馈数据写入 `data/feedback.jsonl`。
- 服务重启后会自动恢复文献库状态与文献库级历史对话。
- 若需彻底清空缓存，可调用 `POST /api/library/reset` 并传 `{"confirm":"yes"}`。

## 后续可扩展方向

1. 论文来源（如 arXiv 链接）一键拉取
2. 文献库持久化（当前重启后内存缓存会清空）
3. 对比结果的评分与实验笔记
4. 账号与协作（若有多用户需求）
